...
print("comment=linegraph-pooling, full")
...
for run in tqdm(range(runs), desc="runs", total=runs):
    ...
    for epoch in tqdm(range(1, 1 + epochs), "epochs", total=epochs):
        ...
        if epoch % eval_steps == 0:
            ...
            torch.save(model.state_dict(), f"models/GNN_model_cc_{city}_{epoch:03d}.pt")
            if valOrig: 
