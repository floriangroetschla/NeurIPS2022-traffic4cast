...
def main(basedir: Path, submission_name: str, model_class, dataset_class, geom=False):
    ...
    gnn_cities = ['models/london_cc.pt', 'models/melbourne_cc.pt', 'models/madrid_cc.pt']

    for cnt, city in enumerate(cities):
        gnn_args = {'compress': False, 'cachedir': Path('cache'), 'edge_attributes': ['parsed_maxspeed', 'importance', 'length_meters']}
